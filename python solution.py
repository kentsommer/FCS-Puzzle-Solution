...
char_count = dict((element, 0) for element in chars)
...
# Count char occurrence in text block
for char in text:
    if char == '\n':
        continue
    char_count[char] += 1

# Add each char to an empty string in sorted order by dictionary value, break when we hit '_'
res_string = ''
for entry in sorted(char_count.keys(), key=char_count.get, reverse=True):
    if entry == '_':
        break
    res_string += entry[0]
